...
#[deprecated]
pub fn try_into_keyword_directly(input: &str) -> Option<Keyword> {
    use Keyword::*; 
    let ans = match input {
        "alignas" => AlignAs, 
        "alignof" => AlignOf, 
...
pub fn try_into_keyword_automaton(input: &str) -> Option<Keyword> {
    let input = input.chars();
    let mut state = automaton::State::default(); 
    let mut rst = None; 
    for i in input {
        let s = state.read(i);  
        match s {
...
pub fn try_into_keyword(input: &str) -> Option<Keyword> {
    try_into_keyword_automaton(input)
}